...
url_list_second_search = ['https://allegro.pl/listing?string=smoczek%20kaszka&bmatch=baseline-eyesa2-dict43-bab-1-4-0205']
second_search_range = 10
links_data_list = []
...
def get_next_page_with_links():
    ...
    url = url_list_second_search[0]
    if second_search_range == 0:
        return;
    else:
        for i in range(second_search_range):
            ...
            print(next_page_url)
            url_list_second_search.append(next_page_url)
            url = next_page_url
...
for url in url_list_second_search:
    get_links(url)

# Create a json file to store the page data
with open('stored_links_second_search.json', 'w', encoding="utf-8") as stored_links:
